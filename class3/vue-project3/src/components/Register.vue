<template>
    <div class="Register">
        ...
        <p class="input_p">确认密码:
            <input :class="{ empty: hasconfirmpassword }" type="password" v-model="confirmpassword">
        </p>
...
export default {
    ...
    methods: {
        register() {
            ...
            if (!(this.hasusername || this.haspassword || this.hasconfirmpassword)){

                //发送注册请求至后端接口进行验证//示例代码：假设后端接口地址为http://localhost:3000/
                axios.post('http://localhost:8082/web_ssm_vue_3_war_exploded/user/Register?username=' + this.username + '&password=' + this.password + '&confirmpassword='+this.confirmpassword)
                    .then(response => {
                        //注册成功处理逻辑
